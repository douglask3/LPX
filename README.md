# Welcome to LPX!
# LPX is the lastest DGVM in the LPJ suite of models. It is a develoment of LPJ-SPITFIRE (Thonicke et al, 2010), with some improvement in the fire commponent, documented in Prentice et al (2011).
# To run the model, you iwll have to have the following installed:
#   1) fortran compiler - to compile the main, sciencey bit of the model. At Macquarie, we use gfortran
#   2) c++ compiler - to comiple the driver (I/O) bit. We use GCC c++
...
#   3) FCOPTIONS = <relevent fortran options, including option for pre-compiling>. For gfrotran, these are typically -x -f77 -cpp-input
#   4) CPPOPTIONS: <you c++ libraries>
#   5) cxx = <you c++ compiler>
#   6) cxxoptions = <relevet c++ options. Typically -o3>
#   7) You will finaly need to change the 2nd path listed on the line "LDLIBS" to point to you talky c++ fortran file
#  
